...
const portableTextComponents = {
  block: {
    normal: (_: any, { slots }: any) => h('p', { class: 'text-2xl' }, slots.default?.()),
  },        
}
</script>
<template>
        ...
        <article class="stack-panel px-12 pt-36 pb-12 flex flex-col relative h-auto">
            ...
            <section v-if="sousSection" class="w-full flex rounded-3xl p-10 flex-col gap-10 min-h-0" :style="{ 'background-color': `var(${color2})` }">
                ...
                <section class="w-full aspect-2/1 flex">
                    <div class="flex-1 h-full bg-[#000000]"><SectionElements @select="selector = $event" :subject="subject" :sectionNumber="sectionNumber" :game="game"/></div>
                    <div class="flex-1 h-full relative bg-[#000000]">
                        <img class="w-full h-full object-cover object-center absolute top-0 left-0 transition-all duration-300 opacity-30" v-if="list[selector]?.image?.asset?.url" :src="list[selector]?.image?.asset?.url" :alt="list[selector]?.image?.alt">
